...
/**
 * 组件属性说明
 * @property {Object} data 需要下载的表格数据
 *{
 *     data, dataTable的data
 *     columns, dataTable的columns
 *     fileName, 文件名，自动加上时间戳
 * }
 * @property {arrray} config 文件格式 ['csv', 'xls'] 目前只支持这种
 * @property {string} tips 提示 放在表格第一行，不设或设空时忽略
 */
  ...
  downloadHandel(type) {
    ...
    for (let i = 0; i < this.props.data.data.length; i++) {
      ...
      for (let j = 0; j < this.props.data.columns.length; j++) {
        const column = this.props.data.columns[j];
        const dataRow = this.props.data.data[i];
